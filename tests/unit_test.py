...
def test_api_key(api_key):
    # 1. Vérifie que la fixtureapi key elle-même a bien renvoyé la valeur
    assert api_key == "MOCK_KEY1234"


def test_channel_handle(channel_handle):    
    # 2. Vérifie que la fixture channel handle elle-même a bien renvoyé la valeur
    assert channel_handle == "FROMAGE"


def test_postgres_conn(mock_postgres_conn_vars):    
    # 3. Vérifie que la fixture database connexion elle-même a bien renvoyé la valeur
...
def test_dags_integrity(dagbag):    
    # 4. Vérifie que la fixture dag bag elle-même a bien renvoyé la valeur
    # 1.
